...
if (reduceMotion) {
  ...
  const tick = (now) => {
    const progress = Math.min(100, Math.floor(((now - started) / 500) * 100));
...
const onScroll = () => {
  ...
  if (!reduceMotion)
    parallaxItems.forEach(
      (item) =>
        (item.style.translate = `0 ${top * Number(item.dataset.speed)}px`),
    );
};
...
if (window.matchMedia("(pointer:fine)").matches && !reduceMotion) {
  ...
  document.querySelectorAll(".magnetic").forEach((item) => {
    item.addEventListener("pointermove", (event) => {
      ...
      item.style.transform = `translate(${(event.clientX - rect.left - rect.width / 2) * 0.14}px, ${(event.clientY - rect.top - rect.height / 2) * 0.14}px)`;
    });
